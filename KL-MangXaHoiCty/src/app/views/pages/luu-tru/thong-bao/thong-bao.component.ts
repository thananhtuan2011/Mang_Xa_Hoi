...
import { ThongBaoDataSource } from '../DataSource/thongbao.datasource';
  ...
  ngOnInit() {
		...
		**/
		merge(this.sort.sortChange, this.paginator.page)
			...
			.subscribe();
		// Init DataSource
		this.dataSource = new ThongBaoDataSource(this.service);
	...
	loadDataList() {
		...
		this.dataSource.loadList_TB(queryParams);
		setTimeout(x => {
			this.loadPage();
		}, 500)
	}
	loadPage() {
        ...
        this.dataSource.loadList_TB(queryParams1);
			}
			else {
				const queryParams1 = new QueryParamsModelNew(
					this.filterConfiguration(),
