...
@Component({
  selector: 'kt-phong-ky-thuat',
  templateUrl: './phong-ky-thuat.component.html',
  styleUrls: ['./phong-ky-thuat.component.scss']
})
export class PhongKyThuatComponent implements OnInit {
  ...
  ngOnInit() {
	...
	loadDataList() {
		...
		this.dataSource.loadList_NV(1,queryParams);
		setTimeout(x => {
			this.loadPage();
		}, 500)
	}
	loadPage() {
        ...
        this.dataSource.loadList_NV(1,queryParams1);
			}
			else {
				const queryParams1 = new QueryParamsModelNew(
					this.filterConfiguration(),
